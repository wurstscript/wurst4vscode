...
- When adding a new editable binary format, mirror this: a casc-ts parser+serializer with a byte-exact round-trip test, a provider built on `EditableBinaryEditorProvider`, and a serialize→re-parse→compare safety gate inside `serialize`. The remaining hand-rolled providers (`.w3r`, `.mmp`, `.w3i`, objmod) should be ported to the base rather than copied.
