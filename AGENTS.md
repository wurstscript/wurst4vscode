...
- Both are hand-made `node_modules` symlinks, not `package.json` dependencies, so `npm ci` (which wipes `node_modules`) removes them. Re-create the links after any clean install.
...
- Avoid introducing unrelated formatting churn in JSON/TS files. There is no `.gitattributes` and sources mix LF and CRLF (check with `git ls-files --eol`), so scripted edits must keep each file's own line endings. Python on Windows writes CRLF by default and turns a one-line change into a whole-file diff.
...
- **src/features/blpPreview.ts** — VS Code custom editor provider + webview HTML for BLP/DDS/TGA/MDX. The page script is **src/webview/blpPreviewWebview.ts** (loaded after `mdxViewer.js`, driving `window.War3Viewer`), styled by **src/webview/blpPreview.css**.
...
- **src/features/webviewUtils.ts** — host-side `makeNonce()` + `escapeHtml()`. The browser-side twin is **src/webview/webviewUtils.ts** (`esc`, `base64ToBytes`, `renderWc3Colors`), shared by every bundle including the thumbnail worker; import from it rather than re-declaring helpers in a bundle.

- **Webview code belongs in `src/webview/` bundles, not in template literals.** Stylesheets are real `.css` files imported by the host (`import CSS from '../webview/x.css'`, inlined by webpack `asset/source` and by the test loader). Scripts are webpack entries in `webpack.config.js` (`viewerConfig.entry`), loaded via `webview.asWebviewUri(dist/webview/<name>.js)` under a nonce, with the page's data handed over through a small `window.__X_INITIAL__ = ${scriptSafeJson(data)}` script carrying the same nonce; the CSP must not fall back to `script-src 'unsafe-inline'`. `wpmEditorWebview.ts` / `wpmPreview.ts` / `wpmEditor.css` is the reference set. `scripts/test-vsix-contents.js` reads the required bundles from `webpack.config.js`. Remaining inline scripts to convert: map preview, code asset browser (`assetLinks.ts`), map-data editors and the `.shd` preview, sound player, `ICON_LAZYLOAD_SCRIPT` (doo preview), and the objmod `__OBJMOD_INITIAL__` handoff.

- **src/features/webviewShared.ts** — `buildPage()` (every page goes through it), `sep()`, the CSP constants (`STATIC_CSP`, `INLINE_SCRIPT_CSP`, `PREVIEW_ICON_CSP`) and `scriptSafeJson()`. It prepends **src/webview/base.css**, the single theme: the VS Code token map (`--fg`, `--muted`, `--accent`, `--error`, `--warn`, `--focus`, `--shadow`, `--widget-*`, `--btn-*`, `--input-*`, …) and the shared components (`.wv-header`, `.wv-toolbar`, `.wv-btn`, `.wv-btn-secondary`/`-primary`, `.wv-input`, `.wv-badge`, `.wv-dirty`, `.wv-sep`, `.wv-scroll`, `.wv-loading-overlay`, `.wv-state`). Tabular read-only pages (trigger, doo, map-data) also append the opt-in `DATA_PAGE_CSS` (`src/webview/dataPage.css`) for tables, `.error`/`.warn` banners, `.empty`, `.count`.
  Only `base.css` reads `--vscode-*` variables; everything else uses its tokens (add a semantic token there when a new VS Code colour is needed) and never redefines one. `testThemeTokensHaveOneHome` in `scripts/test-webview.js` fails on a raw lookup, a redefinition, or a self-referencing custom property. Viewer-specific CSS goes in that viewer's own `.css` file, not in `base.css`.
...
Before adding new webview CSS that should be consistent across viewers, add it to `base.css`.
...
- **Inline model preview:** to embed a 3D model render in a webview (not a separate window), load `dist/webview/mdxViewer.js` via `webview.asWebviewUri` (requires `extensionUri` on the provider, `dist/webview` in `localResourceRoots`, and `${webview.cspSource}` in `script-src`). A page that still has un-nonced inline scripts (objmod, the code asset picker) must keep `'unsafe-inline'` and must not add a nonce: a nonce in `script-src` makes the browser ignore `'unsafe-inline'`. `blpPreview.ts` loads the viewer under a nonce because all its scripts carry it. Host side: `preview/modelPreviewHost.ts` `postModelToWebview` / `postTexturesToWebview`; the viewer posts `requestTextures` back. Route the icon/thumbnail/texture messages (`loadObjectIcon`, `loadModelThumb`, `requestTextures`, `modelThumbRendered`, `modelThumbFailed`) through `handleModelThumbMessage` rather than re-dispatching them per page. The objmod editor's `#mpv-box` is the reference implementation.
...
- **Objmod asset-browser model thumbnails:** visible model cards should enter a pending/spinner state immediately and stay there until the thumbnail is either loaded or decisively marked missing (`?`). Generation must drain visible thumbnails in DOM order, one complete thumbnail lifecycle at a time: host resolve -> render in the `mdxThumbnailWorker` bundle (a Blob worker; there is no main-thread fallback) -> cache/write or missing decision -> next item. Do not pre-resolve/render later visible models in parallel, and do not add fixed inter-thumbnail idle delays after a thumbnail has finished. Cancel queued work only when a thumbnail scrolls out of view before it starts; when it returns, re-observe/requeue it. Large models are not skipped by size (a guard in `scripts/test-webview.js` enforces this); keep them renderable by bounding texture size instead. Use `WURST_MODEL_THUMB_DISABLE_CACHE=1` for local validation so tests measure actual generation rather than cached webps.
...
- Compile TypeScript (`npx tsc -p . --noEmit`) after command or API wiring changes. `src/webview/**` is excluded from that project (and from ESLint); type-check bundle changes with `npx tsc -p tsconfig.webview.json --noEmit`.
