...
function updateSeed(s: number): number {
...
export const CRYPTO_TABLE: Int32Array[] = Array.from({ length: 5 }, () => new Int32Array(256));
...
(function buildTable() {
    ...
    for (let v = 0; v < 256; v++) {
        for (let t = 0; t < 5; t++) {
            ...
            CRYPTO_TABLE[t][v] = hi | lo; // result is signed int32 (JS bitwise always returns int32)
        }
    }
})();
 ...
 */
export function mpqHash(name: string, type: 0 | 1 | 2 | 3): number {
    const lut = CRYPTO_TABLE[type];
 ...
 */
export function mpqDecrypt(buf: Uint8Array, key: number): void {
    const encLut = CRYPTO_TABLE[4];
...
export const KEY_HASH_TABLE = mpqHash('(hash table)', 3);

/** Decryption key for the MPQ block table. */
export const KEY_BLOCK_TABLE = mpqHash('(block table)', 3);
