...
import { mpqHash, mpqDecrypt, KEY_HASH_TABLE, KEY_BLOCK_TABLE } from './crypto';
...
interface Block {
    filePos: number;        // offset from archive header start
...
interface HashEntry {
    ...
    key2: number;
    locale: number;
    blockIndex: number; // raw uint32 (may be HASH_ENTRY_EMPTY/DELETED)
}
 ...
 */
function decompressSector(sector: Uint8Array, compressedSize: number, uncompressedSize: number): Uint8Array {
    ...
    if (compressionType & COMP_IMPLODE) {
        ...
        return explode(sector, uncompressedSize, 1);
    }

    // Unsupported compression types for data files
    const unsupported: string[] = [];
    if (compressionType & COMP_HUFFMAN) unsupported.push('Huffman');
    if (compressionType & COMP_BZIP2)   unsupported.push('bzip2');
    if (compressionType & COMP_ADPCM1C) unsupported.push('ADPCM-1ch');
    if (compressionType & COMP_ADPCM2C) unsupported.push('ADPCM-2ch');
...
export class MpqReader {
    ...
    private readonly discBlockSize: number;
    private readonly hashTable: HashEntry[];
    private readonly hashTableSize: number;
    private readonly blockTable: Block[];
    private _listfile: string[] | undefined;

    private constructor(
        buf: Buffer,
        headerOffset: number,
        discBlockSize: number,
        hashTable: HashEntry[],
        blockTable: Block[],
    ) {
        this.buf = buf;
    ...
    private static findHeader(buf: Buffer): number {
    ...
    private static readHashTable(buf: Buffer, headerOffset: number, hashPos: number, hashSize: number): HashEntry[] {
        ...
        mpqDecrypt(copy, KEY_HASH_TABLE);
    ...
    private static readBlockTable(buf: Buffer, headerOffset: number, blockPos: number, blockCount: number): Block[] {
        ...
        mpqDecrypt(copy, KEY_BLOCK_TABLE);
    ...
    private findBlockIndex(name: string): number | null {
        const offset = mpqHash(name, 0) >>> 0; // unsigned
        const key1   = mpqHash(name, 1);
        const key2   = mpqHash(name, 2);
    ...
    readFile(name: string): Buffer {
        ...
        if (hasFlag(block.flags, FLAG_ENCRYPTED)) {
            ...
            baseKey = mpqHash(pathlessName, 3);
    ...
    private sectorCount(normalSize: number): number {
        return Math.ceil(normalSize / this.discBlockSize) + 1;
    }

    private extractSingleUnit(blockData: Buffer, block: Block, encrypted: boolean, baseKey: number): Buffer {
        const arr = Buffer.from(blockData);
        if (encrypted) {
            mpqDecrypt(arr, baseKey);
        }
        if (hasFlag(block.flags, FLAG_COMPRESSED)) {
    ...
    private extractImploded(blockData: Buffer, block: Block, encrypted: boolean, baseKey: number): Buffer {
        const sc = this.sectorCount(block.normalSize);
        const sotBytes = sc * 4;
        const sotRaw = Buffer.from(blockData.subarray(0, sotBytes));
        if (encrypted) {
            mpqDecrypt(sotRaw, (baseKey - 1) | 0);
        }
    ...
    private extractCompressed(blockData: Buffer, block: Block, encrypted: boolean, baseKey: number): Buffer {
        const sc = this.sectorCount(block.normalSize);
        const sotBytes = sc * 4;
        const sotRaw = Buffer.from(blockData.subarray(0, sotBytes));
        if (encrypted) {
     ...
     */
    listFiles(): string[] {
        if (this._listfile) return this._listfile;
    ...
    /**
     * Return file entries with size metadata from the block table.
     * Only files present in the listfile are included.
     */
    getFilesWithInfo(): MpqFileEntry[] {
        return this.listFiles().map(name => {
            const idx = this.findBlockIndex(name);
            if (idx === null) return { name, normalSize: 0, compressedSize: 0 };
            const b = this.blockTable[idx];
            return { name, normalSize: b?.normalSize ?? 0, compressedSize: b?.compressedSize ?? 0 };
        });
    }
}
